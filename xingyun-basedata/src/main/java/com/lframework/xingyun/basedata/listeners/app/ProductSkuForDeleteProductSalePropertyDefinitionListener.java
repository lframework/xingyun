...
@Component
public class ProductSkuForDeleteProductSalePropertyDefinitionListener implements
  ...
  public void onApplicationEvent(DeleteProductSalePropertyDefinitionEvent event) {
    ...
    Wrapper<ProductSku> queryWrapper = new MPJLambdaWrapper<ProductSku>()
        ...
        .eq(ProductSkuSalePropertyRelation::getPropertyId, event.getEntity().getId());
    if (productSkuService.count(queryWrapper) > 0) {
      throw new DefaultClientException(
          "销售属性：" + event.getEntity().getName() + "已关联SKU，不允许删除！");
    }
  }
}
