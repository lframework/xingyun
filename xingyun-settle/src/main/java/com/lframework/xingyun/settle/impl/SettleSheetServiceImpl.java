...
public class SettleSheetServiceImpl implements ISettleSheetService {
    ...
    @Override
    public PageResult<SettleSheetDto> query(Integer pageIndex, Integer pageSize, QuerySettleSheetVo vo) {
    ...
    public void update(UpdateSettleSheetVo vo) {
        ...
        if (sheet.getStatus() != SettleSheetStatus.CREATED && sheet.getStatus() != SettleSheetStatus.APPROVE_REFUSE) {
        ...
        Wrapper<SettleSheet> updateWrapper = Wrappers.lambdaUpdate(SettleSheet.class)
                ...
                .set(SettleSheet::getRefuseReason, StringPool.EMPTY_STR).eq(SettleSheet::getId, sheet.getId())
    ...
    @Transactional
    @Override
    public void redirectApprovePass(CreateSettleSheetVo vo) {
    ...
    public SettleBizItemDto getBizItem(String id) {
        ...
        result.setTotalUnPayAmount(NumberUtil.sub(checkSheet.getTotalPayAmount(), checkSheet.getTotalPayedAmount(),
        ...
        if (!CollectionUtil.isEmpty(sheetList)) {
            for (SettleCheckSheetDto item : sheetList) {
                ...
                result.setTotalUnPayAmount(NumberUtil
                        .sub(item.getTotalPayAmount(), item.getTotalPayedAmount(), item.getTotalDiscountAmount()));
    ...
    private void create(SettleSheet sheet, CreateSettleSheetVo vo) {
        ...
        for (SettleSheetItemVo itemVo : vo.getItems()) {
            ...
            if (NumberUtil.lt(item.getTotalPayAmount(), 0)) {
                ...
                if (NumberUtil.equal(itemVo.getPayAmount(), 0) && NumberUtil.equal(itemVo.getDiscountAmount(), 0)) {
