...
import com.lframework.xingyun.sc.biz.mappers.ProductStockMapper;
...
import com.lframework.xingyun.sc.facade.vo.stock.QueryProductStockVo;
...
public class ProductStockServiceImpl extends BaseMpServiceImpl<ProductStockMapper, ProductStock>
  ...
  public ProductStockChangeDto addStock(AddProductStockVo vo) {
    ...
    if (count != 1) {
      ProductDto product = productFeignClient.findById(vo.getProductId()).getData();
  ...
  public StockCostAdjustDiffDto stockCostAdjust(StockCostAdjustVo vo) {
    ...
    result.setDiffAmount(NumberUtil.getNumber(NumberUtil.mul(NumberUtil.sub(taxPrice, productStock.getTaxPrice()), productStock.getStockNum()), 2));
