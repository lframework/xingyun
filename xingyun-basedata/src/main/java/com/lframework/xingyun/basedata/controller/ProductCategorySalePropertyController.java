...
import com.lframework.xingyun.basedata.bo.product.category.saleproperty.ProductCategorySalePropertyBo;
...
import com.lframework.xingyun.basedata.vo.product.category.saleproperty.BindProductCategorySalePropertyVo;
import com.lframework.xingyun.basedata.vo.product.category.saleproperty.CreateProductCategorySalePropertyVo;
...
public class ProductCategorySalePropertyController extends DefaultBaseController {
  ...
  @GetMapping("/query")
  public InvokeResult<List<ProductCategorySalePropertyBo>> query(
      @NotBlank(message = "商品分类不能为空！") String categoryId) {

    List<ProductCategorySalePropertyDto> datas = productCategorySalePropertyService.getByCategoryId(
        categoryId);
    if (CollectionUtil.isEmpty(datas)) {
      return InvokeResultBuilder.success(CollectionUtil.emptyList());
    }

    List<ProductCategorySalePropertyBo> results = datas.stream()
        .map(ProductCategorySalePropertyBo::new)
  ...
  @PostMapping("/bind")
  public InvokeResult<Void> bind(@Valid @RequestBody BindProductCategorySalePropertyVo vo) {

    productCategorySalePropertyService.bindExistingProperties(vo.getCategoryId(),
        vo.getPropertyIds());
  ...
  @PostMapping
  public InvokeResult<Void> create(@Valid @RequestBody CreateProductCategorySalePropertyVo vo) {

    CreateProductSalePropertyVo createVo = new CreateProductSalePropertyVo();
    BeanUtils.copyProperties(vo, createVo);

    productCategorySalePropertyService.createPropertyAndBind(vo.getCategoryId(), createVo);
  ...
  public InvokeResult<Void> delete(
    ...
    productCategorySalePropertyService.removeByCategoryIdAndPropertyId(categoryId, propertyId);
