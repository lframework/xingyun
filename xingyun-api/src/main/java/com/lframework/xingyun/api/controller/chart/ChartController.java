...
public class ChartController extends DefaultBaseController {
  ...
  public InvokeResult orderChart() {
    ...
    List<Integer> orderBizTypes = CollectionUtil
        .toList(OrderChartBizType.PURCHASE_ORDER.getCode(), OrderChartBizType.SALE_ORDER.getCode(),
            OrderChartBizType.RETAIL_OUT_SHEET.getCode());
    todayMap.put("order", getTodayChart(orderBizTypes));
    sameMonthMap.put("order", getSameMonthChart(orderBizTypes));

    List<Integer> returnBizTypes = CollectionUtil
        .toList(OrderChartBizType.PURCHASE_RETURN.getCode(),
