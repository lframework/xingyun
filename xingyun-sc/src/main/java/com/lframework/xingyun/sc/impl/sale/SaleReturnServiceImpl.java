...
public class SaleReturnServiceImpl implements ISaleReturnService {
    ...
    @Override
    public PageResult<SaleReturnDto> query(Integer pageIndex, Integer pageSize, QuerySaleReturnVo vo) {
    ...
    public void update(UpdateSaleReturnVo vo) {
        ...
        if (requireOut) {
            ...
            for (SaleReturnDetail detail : details) {
                if (!StringUtil.isBlank(detail.getOutSheetDetailId())) {
                    //先恢复已退货数量
                    saleOutSheetDetailLotService.subReturnNum(detail.getOutSheetDetailId(), detail.getReturnNum());
                }
            }
        }
        ...
        Wrapper<SaleReturn> updateOrderWrapper = Wrappers.lambdaUpdate(SaleReturn.class)
                ...
                .set(SaleReturn::getRefuseReason, StringPool.EMPTY_STR).eq(SaleReturn::getId, saleReturn.getId())
    ...
    public void approvePass(ApprovePassSaleReturnVo vo) {
        ...
        if (!saleConfig.getSaleReturnMultipleRelateOutStock()) {
            ...
            if (saleReturnMapper.selectCount(checkWrapper) > 0) {
                ...
                throw new DefaultClientException("销售出库单号：" + saleOutSheet.getCode() + "，已关联其他销售退货单，不允许关联多个销售退货单！");
            }
        }
        ...
        LambdaUpdateWrapper<SaleReturn> updateOrderWrapper = Wrappers.lambdaUpdate(SaleReturn.class)
                ...
                .set(SaleReturn::getApproveTime, LocalDateTime.now()).eq(SaleReturn::getId, saleReturn.getId())
                .in(SaleReturn::getStatus, statusList);
        ...
        Wrapper<SaleReturnDetail> queryDetailWrapper = Wrappers.lambdaQuery(SaleReturnDetail.class)
                .eq(SaleReturnDetail::getReturnId, saleReturn.getId()).orderByAsc(SaleReturnDetail::getOrderNo);
        List<SaleReturnDetail> details = saleReturnDetailMapper.selectList(queryDetailWrapper);
        for (SaleReturnDetail detail : details) {
    ...
    public void approveRefuse(ApproveRefuseSaleReturnVo vo) {
        ...
        LambdaUpdateWrapper<SaleReturn> updateOrderWrapper = Wrappers.lambdaUpdate(SaleReturn.class)
                ...
                .set(SaleReturn::getRefuseReason, vo.getRefuseReason()).eq(SaleReturn::getId, saleReturn.getId())
    ...
    private void create(SaleReturn saleReturn, CreateSaleReturnVo vo, boolean requireOut) {
        ...
        saleReturn.setPaymentDate(paymentDate.getAllowModify() ? vo.getPaymentDate() : paymentDate.getPaymentDate());
        ...
        if (requireOut) {
            ...
            if (!saleConfig.getSaleReturnMultipleRelateOutStock()) {
                Wrapper<SaleReturn> checkWrapper = Wrappers.lambdaQuery(SaleReturn.class)
                        .eq(SaleReturn::getOutSheetId, saleOutSheet.getId()).ne(SaleReturn::getId, saleReturn.getId());
                if (saleReturnMapper.selectCount(checkWrapper) > 0) {
                    throw new DefaultClientException("销售出库单号：" + saleOutSheet.getCode() + "，已关联其他销售退货单，不允许关联多个销售退货单！");
                }
            }
        ...
        for (SaleReturnProductVo productVo : vo.getProducts()) {
            ...
            if (requireOut && !StringUtil.isBlank(productVo.getOutSheetDetailId())) {
                ...
                saleOutSheetDetailLotService.addReturnNum(productVo.getOutSheetDetailId(), detail.getReturnNum());
            }
        ...
        saleReturn.setDescription(StringUtil.isBlank(vo.getDescription()) ? StringPool.EMPTY_STR : vo.getDescription());
