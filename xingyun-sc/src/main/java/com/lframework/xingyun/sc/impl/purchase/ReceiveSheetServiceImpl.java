...
public class ReceiveSheetServiceImpl implements IReceiveSheetService {
    ...
    @Override
    public PageResult<ReceiveSheetDto> query(Integer pageIndex, Integer pageSize, QueryReceiveSheetVo vo) {
    ...
    @Override
    public PageResult<ReceiveSheetDto> selector(Integer pageIndex, Integer pageSize, ReceiveSheetSelectorVo vo) {
    ...
    public GetPaymentDateDto getPaymentDate(String supplierId) {
        ...
        if (supplier.getManageType() == ManageType.DISTRIBUTION && supplier.getSettleType() == SettleType.ARBITRARILY) {
    ...
    public void update(UpdateReceiveSheetVo vo) {
        ...
        if (sheet.getStatus() != ReceiveSheetStatus.CREATED && sheet.getStatus() != ReceiveSheetStatus.APPROVE_REFUSE) {
        ...
        if (requirePurchase) {
            ...
            for (ReceiveSheetDetail detail : details) {
                if (!StringUtil.isBlank(detail.getPurchaseOrderDetailId())) {
                    //先恢复已收货数量
                    purchaseOrderDetailService.subReceiveNum(detail.getPurchaseOrderDetailId(), detail.getOrderNum());
                }
            }
        }
        ...
        Wrapper<ReceiveSheet> updateOrderWrapper = Wrappers.lambdaUpdate(ReceiveSheet.class)
                ...
                .set(ReceiveSheet::getRefuseReason, StringPool.EMPTY_STR).eq(ReceiveSheet::getId, sheet.getId())
    ...
    public void approvePass(ApprovePassReceiveSheetVo vo) {
        ...
        if (!purchaseConfig.getReceiveMultipleRelatePurchase()) {
            ...
            if (receiveSheetMapper.selectCount(checkWrapper) > 0) {
                ...
                throw new DefaultClientException("采购订单号：" + purchaseOrder.getCode() + "，已关联其他采购收货单，不允许关联多个采购收货单！");
            }
        }
        ...
        LambdaUpdateWrapper<ReceiveSheet> updateOrderWrapper = Wrappers.lambdaUpdate(ReceiveSheet.class)
                ...
                .set(ReceiveSheet::getApproveTime, LocalDateTime.now()).eq(ReceiveSheet::getId, sheet.getId())
                .in(ReceiveSheet::getStatus, statusList);
        ...
        Wrapper<ReceiveSheetDetail> queryDetailWrapper = Wrappers.lambdaQuery(ReceiveSheetDetail.class)
                .eq(ReceiveSheetDetail::getSheetId, sheet.getId()).orderByAsc(ReceiveSheetDetail::getOrderNo);
        List<ReceiveSheetDetail> details = receiveSheetDetailMapper.selectList(queryDetailWrapper);
        for (ReceiveSheetDetail detail : details) {
    ...
    public void approveRefuse(ApproveRefuseReceiveSheetVo vo) {
        ...
        LambdaUpdateWrapper<ReceiveSheet> updateOrderWrapper = Wrappers.lambdaUpdate(ReceiveSheet.class)
                ...
                .set(ReceiveSheet::getRefuseReason, vo.getRefuseReason()).eq(ReceiveSheet::getId, sheet.getId())
    ...
    @Override
    public List<ReceiveSheetDto> getApprovedList(String supplierId, LocalDateTime startTime, LocalDateTime endTime,
    ...
    private void create(ReceiveSheet sheet, CreateReceiveSheetVo vo, boolean receiveRequirePurchase) {
        ...
        for (ReceiveProductVo productVo : vo.getProducts()) {
            ...
            totalAmount = NumberUtil
                    .add(totalAmount, NumberUtil.mul(productVo.getPurchasePrice(), productVo.getReceiveNum()));
            ...
            if (receiveRequirePurchase && !StringUtil.isBlank(productVo.getPurchaseOrderDetailId())) {
                ...
                purchaseOrderDetailService.addReceiveNum(productVo.getPurchaseOrderDetailId(), detail.getOrderNum());
            }
