...
import com.lframework.xingyun.settle.biz.mappers.CustomerSettleCheckSheetMapper;
import com.lframework.xingyun.settle.biz.service.ICustomerSettleCheckSheetDetailService;
import com.lframework.xingyun.settle.biz.service.ICustomerSettleCheckSheetService;
import com.lframework.xingyun.settle.biz.service.ICustomerSettleFeeSheetService;
import com.lframework.xingyun.settle.biz.service.ICustomerSettlePreSheetService;
import com.lframework.xingyun.settle.facade.dto.check.customer.CustomerSettleCheckBizItemDto;
...
import com.lframework.xingyun.settle.facade.enums.CustomerSettleCheckSheetCalcType;
...
import com.lframework.xingyun.settle.facade.vo.check.customer.ApprovePassCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.ApproveRefuseCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.BatchApprovePassCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.BatchApproveRefuseCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.CreateCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.CustomerSettleCheckSheetItemVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.QueryCustomerSettleCheckSheetVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.QueryCustomerUnCheckBizItemVo;
import com.lframework.xingyun.settle.facade.vo.check.customer.UpdateCustomerSettleCheckSheetVo;
...
public class CustomerSettleCheckSheetServiceImpl extends
  ...
  @Autowired
  private SaleOutSheetFeignClient saleOutSheetFeignClient;

  @Autowired
  private SaleReturnFeignClient saleReturnFeignClient;
  ...
  public CustomerSettleCheckBizItemDto getBizItem(String id,
    ...
    switch (bizType) {
      case OUT_SHEET: {
        SaleOutSheet outSheet = saleOutSheetFeignClient.getById(id).getData();
      ...
      case SALE_RETURN: {
        SaleReturn saleReturn = saleReturnFeignClient.getById(id).getData();
  ...
  public void setBizItemUnSettle(String id, CustomerSettleCheckSheetBizType bizType) {
    ...
    switch (bizType) {
      case OUT_SHEET: {
        int count = saleOutSheetFeignClient.setUnSettle(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
      ...
      case SALE_RETURN: {
        int count = saleReturnFeignClient.setUnSettle(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
              "单号：" + item.getCode() + "，" + bizType.getDesc() + "已结算，业务无法进行！");
        }
  ...
  public void setBizItemPartSettle(String id, CustomerSettleCheckSheetBizType bizType) {
    ...
    switch (bizType) {
      case OUT_SHEET: {
        int count = saleOutSheetFeignClient.setPartSettle(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
              "单号：" + item.getCode() + "，" + bizType.getDesc() + "已结算，业务无法进行！");
        }
      ...
      case SALE_RETURN: {
        int count = saleReturnFeignClient.setPartSettle(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
              "单号：" + item.getCode() + "，" + bizType.getDesc() + "已结算，业务无法进行！");
        }
  ...
  public void setBizItemSettled(String id, CustomerSettleCheckSheetBizType bizType) {
    ...
    switch (bizType) {
      case OUT_SHEET: {
        int count = saleOutSheetFeignClient.setSettled(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
      ...
      case SALE_RETURN: {
        int count = saleReturnFeignClient.setSettled(id).getData();
        if (count != 1) {
          throw new DefaultClientException(
              "单号：" + item.getCode() + "，" + bizType.getDesc() + "已结算，无法重复结算！");
        }
  ...
  public List<CustomerSettleCheckBizItemDto> getUnCheckBizItems(
    ...
    List<SaleOutSheet> outSheetList = saleOutSheetFeignClient.getApprovedList(vo.getCustomerId(),
        vo.getStartTime(),
        vo.getEndTime(), SettleStatus.UN_SETTLE).getData();

    List<SaleReturn> saleReturnList = saleReturnFeignClient.getApprovedList(vo.getCustomerId(),
