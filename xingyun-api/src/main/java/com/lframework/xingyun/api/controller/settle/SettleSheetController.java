...
public class SettleSheetController extends DefaultBaseController {
    ...
    public InvokeResult query(@Valid QuerySettleSheetVo vo) {
        ...
        PageResult<SettleSheetDto> pageResult = settleSheetService.query(getPageIndex(vo), getPageSize(vo), vo);

        List<SettleSheetDto> datas = pageResult.getDatas();

        if (!CollectionUtil.isEmpty(datas)) {
            List<QuerySettleSheetBo> results = datas.stream().map(QuerySettleSheetBo::new).collect(Collectors.toList());
    ...
    public InvokeResult redirectApprovePass(@RequestBody @Valid CreateSettleSheetVo vo) {

        settleSheetService.redirectApprovePass(vo);
    ...
    @PreAuthorize("@permission.valid('settle:sheet:delete')")
    @DeleteMapping("/batch")
    public InvokeResult deleteByIds(@RequestBody @NotEmpty(message = "请选择需要删除的供应商结算单！") List<String> ids) {
