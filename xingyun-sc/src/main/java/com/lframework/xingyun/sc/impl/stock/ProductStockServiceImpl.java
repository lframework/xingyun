...
public class ProductStockServiceImpl implements IProductStockService {
  ...
  public ProductStockChangeDto addStock(AddProductStockVo vo) {
    ...
    if (vo.getTaxAmount() == null) {
      vo.setTaxAmount(isStockEmpty ? vo.getDefaultTaxAmount()
    ...
    addLogWithAddStockVo
        .setUnTaxAmount(NumberUtil.calcUnTaxPrice(vo.getTaxAmount(), vo.getTaxRate()));
    addLogWithAddStockVo.setOriStockNum(productStock.getStockNum());
    addLogWithAddStockVo
        .setCurStockNum(NumberUtil.add(productStock.getStockNum(), vo.getStockNum()).intValue());
  ...
  public ProductStockChangeDto subStock(SubProductStockVo vo) {
    ...
    for (Map.Entry<ProductLotStockDto, Integer> lotEntry : subLotMap.entrySet()) {
      ...
      BigDecimal subTaxAmount = subLotMap.size() - 1 == index ?
          remainTaxAmount :
          NumberUtil
              .getNumber(NumberUtil.mul(num, NumberUtil.div(vo.getTaxAmount(), vo.getStockNum())),
                  2);
      ...
      addLogWithAddStockVo.setOriStockNum(productStock.getStockNum());
      addLogWithAddStockVo
          .setCurStockNum(NumberUtil.sub(productStock.getStockNum(), num).intValue());
      addLogWithAddStockVo.setOriTaxPrice(productStock.getTaxPrice());
      addLogWithAddStockVo.setCurTaxPrice(!reCalcCostPrice ?
          productStock.getTaxPrice() :
          addLogWithAddStockVo.getCurStockNum() == 0 ?
  ...
  public void stockCostAdjust(StockCostAdjustVo vo) {
    ...
    if (!CollectionUtil.isEmpty(lotStocks)) {
      for (ProductLotStockDto lotStock : lotStocks) {
        BigDecimal taxAmount = NumberUtil.getNumber(NumberUtil
            .mul(lotStock.getStockNum(), NumberUtil.sub(taxPrice, productStock.getTaxPrice())), 2);
        BigDecimal unTaxAmount = NumberUtil.getNumber(NumberUtil
                .mul(lotStock.getStockNum(), NumberUtil.sub(unTaxPrice, productStock.getUnTaxPrice())),
            6);
